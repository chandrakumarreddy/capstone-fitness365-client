...
export default function BookSwimmingClass() {
  const searchParams = useSearchParams()[0];
  const id = searchParams.get("id") || "4001";
  ...
  const onSelect = (newValue: Dayjs) => {
    ...
    Modal.confirm({
      ...
      async onOk() {
        await fetch(
          `https://capstone-fitness.up.railway.app/api/trainer/4001`,
          {
            method: "post",
            headers: {
        ...
        <div className="flex gap-12 flex-1 items-start my-6">
          <div className="w-full ">
            ...
            <h3 className="text-2xl font-medium text-center">
              Book Swimming class by selecting date from below Calendar
            </h3>
            {isLoading ? (
